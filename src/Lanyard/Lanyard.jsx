    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group scale={2.25} position={[0, -1.2, -0.05]} onPointerOver={() => hover(true)} onPointerOut={() => hover(false)} onPointerUp={event => (event.target.releasePointerCapture(event.pointerId), drag(false))} onPointerDown={event => (event.target.setPointerCapture(event.pointerId), drag(new THREE.Vector3().copy(event.point).sub(vec.copy(card.current.translation()))))}>
